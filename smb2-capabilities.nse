...
action = function(host,port)
  ...
  for i, dialect in pairs(smb2_dialects) do
    ...
    if status then
      ...
      if smbstate['negotiated_capabilities'] ~= nil then
         local n = smbstate['negotiated_capabilities']
         local s = {}
         if n.encryption ~= nil then
            local ciphers = {}
            for _, name in pairs(n.encryption.ciphers) do
               table.insert(ciphers, name)
            end
            s['Encryption'] = { Ciphers = ciphers }
         end
         if n.preauth_integrity ~= nil then
            local algorithms = {}
            for _, name in pairs(n.preauth_integrity.algorithms) do
               table.insert(algorithms, name)
            end
            s['Preauth Integrity'] = {
               Algorithms = algorithms,
               Salt = stdnse.tohex(n.preauth_integrity.salt, {separator = ":", group = 2})
            }
         end
         if n.compression ~= nil then
            local algorithms = {}
            for _, name in pairs(n.compression.algorithms) do
               table.insert(algorithms, name)
            end
            local flags = {}
            if n.compression.flags & 1 == 1 then
               table.insert(flags, "Chained")
            end
            s['Compression'] = {
               Algorithms = algorithms,
               Flags = flags
         ...
         if n.netname ~= nil then
            s['Netname'] = n.netname
         end
         capabilities["Negotiated Capabilities"] = s
      end
