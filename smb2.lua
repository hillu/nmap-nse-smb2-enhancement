...
local function parse_neg_context(packet)
   local cap = {}
   local p_offset = 1
   while true do
      -- Skip padding if any
      if (p_offset) % 8 ~= 1 then
         p_offset = p_offset + (-p_offset % 8) + 1
      end
      if p_offset + 8 >= #packet then
         break
      end
      local typ, data_len
      typ, data_len, p_offset = string.unpack('<I2 I2 xxxx', packet, p_offset)
      if typ == 0 or data_len == 0 then
         break
      end
      data, p_offset = string.unpack("c"..data_len, packet, p_offset)
      if typ == smb2_values['SMB2_PREAUTH_INTEGRITY_CAPABILITIES'] then
         local rec = { algorithms = {} }
         local alg_count, salt_len
         local offset = 1
         alg_count, salt_len, offset = string.unpack('<I2 I2', data, offset)
         for _ = 1, alg_count do
            local alg
            alg, offset = string.unpack("<I2", data, offset)
            if alg == 1 then
               rec.algorithms[1] = "SHA-512"
            else
               rec.algorithms[alg] = string.format("unknoan(%d)", alg)
            end
         end
         rec['salt'], offset = string.unpack("c"..salt_len, data, offset)
         cap.preauth_integrity = rec
      elseif typ == smb2_values['SMB2_ENCRYPTION_CAPABILITIES'] then
         local rec = { ciphers = {} }
         local cipher_count
         local offset = 1
         cipher_count, offset = string.unpack('<I2', data, offset)
         for _ = 1, cipher_count do
            local cipher
            cipher, offset = string.unpack('<I2', data, offset)
            if cipher == 1 then
               rec.ciphers[cipher] = "AES-128-CCM"
            elseif cipher == 2 then
               rec.ciphers[cipher] = "AES-128-GCM"
            else
               rec.ciphers[cipher] = string.format("unknoan(%d)", cipher)
            end
         end
         cap.encryption = rec
      elseif typ == smb2_values['SMB2_COMPRESSION_CAPABILITIES'] then
         local rec = { algorithms = {} }
         local alg_count
         local offset = 1
         alg_count, rec['flags'], offset =  string.unpack('<I2 xx I4', data, offset)
         for _ = 1, alg_count do
            local alg
            alg, offset = string.unpack("<I2", data, offset)
            if alg == 0 then
               rec.algorithms[0] = "None"
            elseif alg == 1 then
               rec.algorithms[1] = "LZNT1"
            elseif alg == 2 then
               rec.algorithms[2] = "LZ77"
            elseif alg == 3 then
               rec.algorithms[3] = "LZ77+Huffman"
            elseif alg == 4 then
               rec.algorithms[4] = "Pattern_V1"
            else
               rec.algorithms[alg] = string.format("unknoan(%d)", alg)
            end
         end
         cap.compression = rec
      elseif typ == smb2_values['SMB2_NETNAME_NEGOTIATE_CONTEXT_ID'] then
         -- TODO decode UTF-16?
         cap['netname'] = data
      else
         stdnse.debug("SMB2: unknown NEGOTIATE_CONTEXT type 0x04d", typ)
      end
   end
   return cap
end
...
function negotiate_v2(smb, overrides)
  ...
  if is_0311 then
    ...
    padding_data = string.rep("\0", (8 - total_data % 8) % 8)
    data = data .. padding_data
    context_data = string.pack("<I2 I2 I2 I16 I16",
    ...
    -- SMB2_COMPRESSINO_CAPABILITIES
    context_data = string.pack("<I2 I2 I4 I2 I2 I2 I2 I2",
                               0x0005, -- Count
                               0x0000, -- Padding
                               0x00000000, -- Flags
                               0x0004, -- Pattern_V1
    ...
    data = data .. string.pack("<I2 I2 I4",
                    smb2_values['SMB2_COMPRESSION_CAPABILITIES'], -- ContextType
                    #context_data,                                -- DataLength
                    0x0                                           -- Reserved
    ) .. context_data
  ...
  data_structure_size, smb['security_mode'], smb['dialect'],
    ...
    smb['start_time'],
    security_buffer_offset, security_buffer_length, neg_context_offset
  ...
  if security_buffer_offset ~= 0 and security_buffer_length ~= 0 then
     local security_buffer = string.sub(data, security_buffer_offset + 1 - #header,
                                        security_buffer_length)
  end

  if smb['dialect'] == 0x311 then
     smb['negotiated_capabilities'] = parse_neg_context(string.sub(data, neg_context_offset + 1 - #header))
  end
